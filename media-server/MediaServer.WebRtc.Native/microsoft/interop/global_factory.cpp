...
void GlobalFactory::ForceShutdown() noexcept
{
    ...
    {
        RTC_LOG(LS_ERROR) << "Failed to shutdown library with exception: " << e.what();
    }
    catch(...)
    {
...
bool GlobalFactory::TryShutdown() noexcept
{
    ...
    {
        RTC_LOG(LS_ERROR) << "Failed to attempt to shutdown library with exception: " << e.what();
    }
    catch(...)
    {
        RTC_LOG(LS_ERROR) << "Failed to attempt to shutdown library due to unknown exception.";
    }
...
RefPtr<GlobalFactory> GlobalFactory::GetInstancePtrImpl(bool ensureInitialized)
{
    GlobalFactory *const factory = GetInstance();
    std::scoped_lock lock(factory->init_mutex_);
    if(factory->peer_factory_)
...
rtc::scoped_refptr<webrtc::PeerConnectionFactoryInterface> GlobalFactory::
...
mrsResult GlobalFactory::InitializeImplNoLock()
{
    ...
    auto dispatcherQueue = wrapper::impl::org::webRtc::EventQueue::toWrapper(dispatcher);

    // Setup the WebRTC library
    {
        auto libConfig = std::make_shared<wrapper::impl::org::webRtc::WebRtcLibConfiguration>();
    ...
    {
        auto factoryConfig =
            std::make_shared<wrapper::impl::org::webRtc::WebRtcFactoryConfiguration>();
    ...
    signaling_thread_->SetName("WebRTC signaling thread", signaling_thread_.get());
    ...
    peer_factory_ = webrtc::CreatePeerConnectionFactory(
        ...
        std::unique_ptr<webrtc::VideoEncoderFactory>(new webrtc::MultiplexEncoderFactory(
            absl::make_unique<webrtc::InternalEncoderFactory>())),
        std::unique_ptr<webrtc::VideoDecoderFactory>(new webrtc::MultiplexDecoderFactory(
    ...
    peer_factory_ = webrtc::CreatePeerConnectionFactory(
        ...
        nullptr);
#endif // defined(WINUWP)
    return (peer_factory_.get() != nullptr ? Result::kSuccess : Result::kUnknownError);
}
...
{
    RTC_LOG(LS_INFO) << "mr-webrtc alive objects report for " << alive_objects_.size()
                     << " objects:";
    int i = 0;

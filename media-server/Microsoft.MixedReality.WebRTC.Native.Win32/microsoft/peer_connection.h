...
class PeerConnection : public TrackedObject
{
  public:
    ...
    static ErrorOr<RefPtr<PeerConnection>> create(const PeerConnectionConfiguration &config,
    ...
    using IceCandidateReadytoSendCallback = Callback<const char *, int, const char *>;
    ...
    virtual void RegisterIceStateChangedCallback(IceStateChangedCallback &&callback) noexcept = 0;
    ...
    virtual void RegisterConnectedCallback(ConnectedCallback &&callback) noexcept = 0;
    ...
    virtual void RegisterTrackAddedCallback(TrackAddedCallback &&callback) noexcept = 0;
    ...
    virtual void RegisterTrackRemovedCallback(TrackRemovedCallback &&callback) noexcept = 0;
    ...
    virtual void RegisterRemoteVideoFrameCallback(
        const Callback<const webrtc::VideoFrame &> &callback) noexcept = 0;
    ...
    virtual void AddLocalVideoTrack(
        rtc::scoped_refptr<webrtc::VideoTrackInterface> video_track) = 0;

    /// Remove a local video track from the peer connection.
    /// The underlying RTP sender/transceiver are kept alive but inactive.
    virtual webrtc::RTCError RemoveLocalVideoTrack(LocalVideoTrack &video_track) noexcept = 0;
    ...
    virtual void RemoveLocalVideoTracksFromSource(ExternalVideoTrackSource &source) noexcept = 0;
    ...
    virtual void RegisterLocalAudioFrameCallback(AudioFrameReadyCallback callback) noexcept = 0;
    ...
    virtual void RegisterRemoteAudioFrameCallback(AudioFrameReadyCallback callback) noexcept = 0;
    ...
    /// and added locally.
    using DataChannelAddedCallback = Callback<mrsDataChannelInteropHandle, DataChannelHandle>;

    /// Callback invoked when a data channel is removed from the remote peer and
    /// removed locally.
    using DataChannelRemovedCallback = Callback<mrsDataChannelInteropHandle, DataChannelHandle>;
    ...
    /// registered at a time.
    virtual void RegisterDataChannelAddedCallback(DataChannelAddedCallback callback) noexcept = 0;

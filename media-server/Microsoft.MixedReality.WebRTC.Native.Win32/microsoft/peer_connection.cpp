...
#include <functional>
    ...
    (WINAPI_FAMILY == WINAPI_FAMILY_APP) /* UWP app */ && defined(_WIN32_WINNT_WIN10) &&           \
...
#define RETURN_IF_ERROR(...)                                                                       \
    ...
    RETURN_IF_ERROR(holo_space_statics->get_IsAvailable(&is_holo_space_available));
    ...
    RETURN_IF_ERROR(GetActivationFactory(
        HStringReference(RuntimeClass_Windows_Graphics_Holographic_HolographicDisplay).Get(),
...
Microsoft::MixedReality::WebRTC::Error ErrorFromRTCError(const webrtc::RTCError &error)
{
    return Microsoft::MixedReality::WebRTC::Error(ResultFromRTCErrorType(error.type()),
                                                  error.message());
}
...
class PeerConnectionImpl : public PeerConnection, public webrtc::PeerConnectionObserver
{
  public:
    ...
    PeerConnectionImpl(RefPtr<GlobalFactory> global_factory,
        ...
        : PeerConnection(std::move(global_factory)), interop_handle_(interop_handle)
    {
    }
    ...
    void RegisterIceStateChangedCallback(IceStateChangedCallback &&callback) noexcept override
    {
    ...
    void RegisterConnectedCallback(ConnectedCallback &&callback) noexcept override
    {
    ...
    void RegisterTrackAddedCallback(TrackAddedCallback &&callback) noexcept override
    {
        auto lock = std::scoped_lock{track_added_callback_mutex_};
        track_added_callback_ = std::move(callback);
    }

    void RegisterTrackRemovedCallback(TrackRemovedCallback &&callback) noexcept override
    {
    ...
    void RegisterRemoteVideoFrameCallback(
        const Callback<const webrtc::VideoFrame &> &callback) noexcept override
    {
        remote_video_observer_.reset(new VideoFrameObserver(callback));
    }
    ...
    virtual void AddLocalVideoTrack(
        rtc::scoped_refptr<webrtc::VideoTrackInterface> video_track) override;
    webrtc::RTCError RemoveLocalVideoTrack(LocalVideoTrack &video_track) noexcept override;
    void RemoveLocalVideoTracksFromSource(ExternalVideoTrackSource &source) noexcept override;

    void RegisterLocalAudioFrameCallback(AudioFrameReadyCallback callback) noexcept override
    {
    ...
    void RegisterRemoteAudioFrameCallback(AudioFrameReadyCallback callback) noexcept override
    {
    ...
    bool AddLocalAudioTrack(
        rtc::scoped_refptr<webrtc::AudioTrackInterface> audio_track) noexcept override;
    ...
    void RegisterDataChannelAddedCallback(DataChannelAddedCallback callback) noexcept override
    {
        auto lock = std::scoped_lock{data_channel_added_callback_mutex_};
        data_channel_added_callback_ = std::move(callback);
    }

    void RegisterDataChannelRemovedCallback(DataChannelRemovedCallback callback) noexcept override
    {
    ...
    void OnSignalingChange(
        webrtc::PeerConnectionInterface::SignalingState new_state) noexcept override;

    // Triggered when media is received on a new stream from remote peer.
    void OnAddStream(rtc::scoped_refptr<webrtc::MediaStreamInterface> stream) noexcept override;

    // Triggered when a remote peer closes a stream.
    void OnRemoveStream(rtc::scoped_refptr<webrtc::MediaStreamInterface> stream) noexcept override;

    // Triggered when a remote peer opens a data channel.
    void OnDataChannel(
        rtc::scoped_refptr<webrtc::DataChannelInterface> data_channel) noexcept override;
    ...
    void OnIceConnectionChange(
        webrtc::PeerConnectionInterface::IceConnectionState new_state) noexcept override;

    /// Called any time the IceGatheringState changes.
    void OnIceGatheringChange(
        webrtc::PeerConnectionInterface::IceGatheringState new_state) noexcept override;
    ...
    void OnRemoveTrack(rtc::scoped_refptr<webrtc::RtpReceiverInterface> receiver) noexcept override;
  ...
  protected:
    ...
    ConnectedCallback connected_callback_ RTC_GUARDED_BY(connected_callback_mutex_);
    ...
    TrackAddedCallback track_added_callback_ RTC_GUARDED_BY(track_added_callback_mutex_);

    /// User callback invoked when a remote audio or video track is removed.
    TrackRemovedCallback track_removed_callback_ RTC_GUARDED_BY(track_removed_callback_mutex_);
    ...
    std::vector<RefPtr<LocalVideoTrack>> local_video_tracks_ RTC_GUARDED_BY(tracks_mutex_);
    ...
    std::vector<std::shared_ptr<DataChannel>> data_channels_ RTC_GUARDED_BY(data_channel_mutex_);
    ...
    std::unordered_multimap<str, std::shared_ptr<DataChannel>> data_channel_from_label_
        RTC_GUARDED_BY(data_channel_mutex_);
...
class CreateSessionDescObserver : public webrtc::CreateSessionDescriptionObserver
{
  public:
    CreateSessionDescObserver(RefPtr<PeerConnectionImpl> peer_connection)
        : peer_connection_(std::forward<RefPtr<PeerConnectionImpl>>(peer_connection))
    {
    }
...
class SessionDescObserver : public webrtc::SetSessionDescriptionObserver
{
  public:
    ...
    SessionDescObserver(Closure &&callback) : callback_(std::forward<Closure>(callback))
    {
    }
    ...
    {
        RTC_LOG(LS_ERROR) << "Error setting session description: " << error.message();
    }
...
struct SetRemoteSessionDescObserver : public webrtc::SetRemoteDescriptionObserverInterface
{
  public:
    ...
    template <typename Closure>
    SetRemoteSessionDescObserver(Closure &&callback) : callback_(std::forward<Closure>(callback))
    {
    }
...
IceConnectionState IceStateFromImpl(webrtc::PeerConnectionInterface::IceConnectionState impl_state)
{
    ...
    mrsLocalVideoTrackInteropHandle interop_handle) noexcept
{
    if(IsClosed())
    {
        return Microsoft::MixedReality::WebRTC::Error(Result::kInvalidOperation,
                                                      "The peer connection is closed.");
    }
    auto result = peer_->AddTrack(video_track, {kAudioVideoStreamId});
    if(result.ok())
    {
        RefPtr<LocalVideoTrack> track = new LocalVideoTrack(global_factory_,
                                                            *this,
                                                            std::move(video_track),
                                                            std::move(result.MoveValue()),
                                                            interop_handle);
        {
...
void PeerConnectionImpl::AddLocalVideoTrack(
    rtc::scoped_refptr<webrtc::VideoTrackInterface> video_track)
{
    if(IsClosed())
    {
        throw new std::runtime_error("This peer connection is already closed");
    }
    auto result = peer_->AddTrack(video_track, {kAudioVideoStreamId});
    if(false == result.ok())
    {
        throw new std::runtime_error("Failed adding the track");
    }
    else
    {
        auto params = result.value()->GetParameters();
        params.degradation_preference = webrtc::DegradationPreference::DISABLED;
        result.value()->SetParameters(params);
    }
}

webrtc::RTCError PeerConnectionImpl::RemoveLocalVideoTrack(LocalVideoTrack &video_track) noexcept
{
    rtc::CritScope lock(&tracks_mutex_);
    auto it = std::find_if(local_video_tracks_.begin(),
...
void PeerConnectionImpl::RemoveLocalVideoTracksFromSource(ExternalVideoTrackSource &source) noexcept
{
    if(!peer_)
    {
        return;
    ...
    std::vector<rtc::scoped_refptr<webrtc::RtpSenderInterface>> senders = peer_->GetSenders();
    for(auto &&sender : senders)
    {
        ...
        if(video_track->GetSource() == (webrtc::VideoTrackSourceInterface *)&source)
        {
    ...
    mrsDataChannelInteropHandle dataChannelInteropHandle) noexcept
{
           ...
           peer_->CreateDataChannel(labelString, &config))
    {
        ...
        auto data_channel =
            std::make_shared<DataChannel>(this, std::move(impl), dataChannelInteropHandle);
        {
...
void PeerConnectionImpl::RemoveDataChannel(const DataChannel &data_channel) noexcept
{
    ...
    std::shared_ptr<DataChannel> data_channel_ptr;
    {
        ...
        // already
        auto const it = std::find_if(std::begin(data_channels_),
                                     std::end(data_channels_),
...
void PeerConnectionImpl::OnDataChannelAdded(const DataChannel &data_channel) noexcept
{
    ...
    {
        auto lock = std::scoped_lock{data_channel_mutex_};
        RTC_DCHECK(std::find_if(data_channels_.begin(),
                                data_channels_.end(),
                                [&data_channel](const std::shared_ptr<DataChannel> &other) {
                                    return other.get() == &data_channel;
                                }) != data_channels_.end());
...
bool PeerConnectionImpl::CreateOffer() noexcept
{
    ...
    auto observer = new rtc::RefCountedObject<CreateSessionDescObserver>(this); // 0 ref
...
bool PeerConnectionImpl::SetRemoteDescriptionAsync(const char *type,
                                                   const char *sdp,
                                                   Callback<> callback) noexcept
{
    ...
    peer_->SetRemoteDescription(std::move(session_description), std::move(observer));
    return true;
}
    ...
    rtc::scoped_refptr<webrtc::DataChannelInterface> impl) noexcept
{
    ...
    if(impl->ordered())
    {
        config.flags = (mrsDataChannelConfigFlags)((uint32_t)config.flags |
                                                   (uint32_t)mrsDataChannelConfigFlags::kOrdered);
    }
    if(impl->reliable())
    {
        config.flags = (mrsDataChannelConfigFlags)((uint32_t)config.flags |
    ...
    {
        data_channel_interop_handle = (*create_cb)(interop_handle_, config, &callbacks);
    }

    // Create a new native object
    auto data_channel = std::make_shared<DataChannel>(this, impl, data_channel_interop_handle);
    {
        auto lock = std::scoped_lock{data_channel_mutex_};
        data_channels_.push_back(data_channel);
        if(!label.empty())
...
void PeerConnectionImpl::OnIceCandidate(const webrtc::IceCandidateInterface *candidate) noexcept
{
    auto lock = std::scoped_lock{ice_candidate_ready_to_send_callback_mutex_};
    ...
    /*streams*/) noexcept
{
    ...
    {
        RTC_LOG(LS_INFO) << "+ Track #" << receiver->id() << " with stream #" << stream->id();
    }
    ...
    else if(trackKindStr == webrtc::MediaStreamTrackInterface::kVideoKind)
    {
        ...
        if(auto *sink = remote_video_observer_.get())
        {
            ...
            sink_settings.rotation_applied = true; // no exposed API for caller to handle rotation
...
void PeerConnectionImpl::OnLocalDescCreated(webrtc::SessionDescriptionInterface *desc) noexcept
{
    if(!peer_)
    {
        return;
    ...
    if(cb)
    {
        ...
        observer = new rtc::RefCountedObject<SessionDescObserver>(
            [cb, type = std::move(type), sdp = std::move(sdp)] { cb(type.c_str(), sdp.c_str()); });
    }
    else
    {
        observer = new rtc::RefCountedObject<SessionDescObserver>();
...
webrtc::PeerConnectionInterface::BundlePolicy BundlePolicyToNative(BundlePolicy mrsValue)
{
    ...
    mrsPeerConnectionInteropHandle interop_handle)
{
    ...
    rtc_config.sdp_semantics =
        (config.sdp_semantic == SdpSemantic::kUnifiedPlan ? webrtc::SdpSemantics::kUnifiedPlan
